...
" JT Last Change: 2013/01/25 04:58pm
...
"
" $Log: savevers.vim,v $
" Revision JT 1 2013/25/01 04:58pm  justin thomas
" Since I have set the backup directory to be the same for all my
" files, I want the backup filename to partially reflect the full
" path of the file.  For example, C:\Users\jthoma20\Documents\main.tex
" will be backed up as <BACKUPFOLDER>\C_U_j_D_main.tex.NN.clean
" I added a function s:abbr_path() which computes the string 
" which approximates the fill path of the file.  In the example, this
" is "C_U_j_D_" then I edited the function s:get_version(base,i,dir)
" joint this string on to the beginning of the filename
"
...
"JT the function s:abbr_path(base) approximates the full path
"given by base into a string to be used later in a filename
"For example, s:abbr_path("C:\Users\jthoma20\Documents") will
"return the string "C_U_j_D_"
"We do this by taking the first letter in the path, and every
"letter immediately following a backslash
function! s:abbr_path(base)
   "l:i will run through the indices we want to extract from
   "the string a:base.  We always take the first letter ('C', 
   "in the example, so we always have the index 0
	let l:i = 0
    "start a list of indices
	let l:list_of_letter_indices = [l:i]
    "the loop and a half construction: our sentinel value is
    "l:i == -1, but I like open conditions, so I put l:i < 1,
    "(l:i = 0 we already have)
	while(1)
       "find the next index location of the backslash in a:base,
       "the l:i + 1 argument tells where to start the search
		let l:i = match(a:base, '\', l:i + 1)
        "check the sentinel value.  i.e. see if the backslash
        "was not found
		if l:i < 1 
			break
		endif
        "adjoin the location of the letter just past this backslash
		call add(l:list_of_letter_indices, l:i + 1)
	endwhile
    "start building the "C_U_j_D_" string
	let l:path = ""
    "loop over the indices and take the substring of length 1 starting
    "at that index.  Recall that all indices are either 0 or just 
    "after a backslash in the given argument string a:base
	for l:index in l:list_of_letter_indices
		let l:path.=strpart(a:base, l:index, 1)."_"
	endfor
    "all done! send her back
    return l:path
endfunction

function! s:get_version(base,i,dir)
   "JT :the "s:abbr_path(a:base) . " part is mine.  Everything else is left
   "alone in this function
   if a:dir =~ "^$" || a:dir =~ "^\\\./$"
      return s:abbr_path(a:base) . fnamemodify(a:base,":p") . s:get_ext(a:i)
   else
      return a:dir . s:abbr_path(a:base) . fnamemodify(a:base,":t") . s:get_ext(a:i)
   endif
endfunction
...
function! s:next_versdir(base)
   while strlen(s:verspath_iterator)
      ...
      if l:comma < 0
         "no comma, means s:verspath_iterator is the path we want
         let l:dir = s:verspath_iterator
         let s:verspath_iterator = ""
      elseif l:comma == 0
         "leading comma: go remove then go to top of loop
         let s:verspath_iterator = strpart(s:verspath_iterator,1)
         continue
      else
         "some comma, not leading: strip off directory before comma
